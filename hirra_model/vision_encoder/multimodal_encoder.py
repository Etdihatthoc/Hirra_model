...
class MultimodalEncoder(nn.Module):
    def __init__(self, ctvit_config: dict, num_fusion_layers: int = 4):
        """
        ...
        #
        self.ct_encoder = CTViT(**ctvit_config, use_vgg_and_gan=False)
        self.pet_encoder = CTViT(**ctvit_config, use_vgg_and_gan=False)
        ...
        for _ in range(num_fusion_layers):
            self.fusion_layers.append(nn.ModuleList([
                CrossAttentionLayer(dim=dim), # CT "hỏi" PET
                CrossAttentionLayer(dim=dim)  # PET "hỏi" CT
            ]))
