    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        try:
            ...
            for i in range(len(batch["patient_ids"])):
                ...
                # Note: outputs may include the input prompt, so we need to extract only new tokens
                # This depends on how model.generate() returns outputs
                if isinstance(outputs, torch.Tensor):
                    # If outputs is a tensor of token IDs
                    pred_ids = outputs[i]
                    # Skip the input prompt tokens
                    prompt_len = attention_mask[i].sum().item()
                    new_ids = pred_ids[prompt_len:]
